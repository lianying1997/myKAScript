...
{
    [ScriptType(name: "M3s绘图+", territorys:[1230],guid: "a1e9d841-0ace-4543-ad13-98b0682b9ca6", version:"0.0.0.3.alpha", author: "Usami From Karlin")]
    public class M3s绘图绘图
    {
        ...
        public void 钢铁月环分摊分散(Event @event, ScriptAccessory accessory)
        {
            ...
            if (@event["ActionId"]== "37850"|| @event["ActionId"] == "37851")
            {
                for (int i = 0; i < 4; i++)
                {
                    ...
                    dp.Owner = sid;
                    dp.TargetObject = accessory.Data.PartyList[i];
                    // dp.Color = (i == index || i == group[index]) ? accessory.Data.DefaultSafeColor : color_lightpurple;
        ...
        public void 玩家自爆范围(Event @event, ScriptAccessory accessory)
        {
            ...
            // accessory.Log.Debug($"检测到玩家自爆范围，阶段为{parse}");
        ...
        public void 地面炸弹爆炸范围(Event @event, ScriptAccessory accessory)
        {
            ...
            // statusCount 0 为短炸弹，1 为长炸弹
            var statusCount = ((IBattleChara)obj).StatusList.Where(status => status.StatusId == 4016).Count();
            
            var pos = JsonConvert.DeserializeObject<Vector3>(@event["SourcePosition"]);
            // accessory.Log.Debug($"检测到地面炸弹自爆范围，statusCount为{statusCount}");
            // accessory.Log.Debug($"对应的位置为{pos}");
            ...
            if (MathF.Abs((pos - bomb_A).Length()) < 0.5) {

                bomb_A_isShort = statusCount == 0;
